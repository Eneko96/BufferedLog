...
(0, node_test_1.beforeEach)(() => {
    logger = new index_js_1.BufferedLogger({ bufferSize: 3 });
});
...
(0, node_test_1.test)("BufferedLogger should correctly use a custom flush function", () => {
    let flushedBuffer = [];
    const customLogger = new index_js_1.BufferedLogger({
        bufferSize: 2,
        customFlush: (buffer) => {
...
(0, node_test_1.test)("BufferedLogger should send messages to a worker if isWorker is true", () => {
    const workerLogger = new index_js_1.BufferedLogger({ bufferSize: 2, isWorker: true });
...
(0, node_test_1.test)("BufferedLogger should properly dispose of the worker", () => {
    const workerLogger = new index_js_1.BufferedLogger({ isWorker: true });
