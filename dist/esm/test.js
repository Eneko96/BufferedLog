...
test("BufferedLogger should flush automatically when bufferSize is reached", () => {
    ...
    assert.strictEqual(logger["buffer"].length, 0, "Buffer should be flushed when full");
});
test("BufferedLogger should correctly use a custom flush function", () => {
    ...
    assert.deepStrictEqual(flushedBuffer, ["Custom 1", "Custom 2"], "Custom flush should receive the correct buffer");
    assert.strictEqual(customLogger["buffer"].length, 0, "Buffer should be cleared after custom flush");
});
test("BufferedLogger should send messages to a worker if isWorker is true", () => {
    ...
    assert.deepStrictEqual(worker.receivedMessages, ["Worker 1", "Worker 2"], "Worker should receive the messages");
});
test("BufferedLogger should not flush before reaching bufferSize", () => {
    logger.log("Msg 1");
    logger.log("Msg 2");
    assert.strictEqual(logger["buffer"].length, 2, "Buffer should hold messages until full");
});
test("BufferedLogger should properly dispose of the worker", () => {
    const workerLogger = new BufferedLogger({ isWorker: true });
    assert(workerLogger["worker"], "Worker should be initialized before disposing");
    workerLogger.dispose();
    assert.strictEqual(workerLogger["worker"], null, "Worker should be null after dispose");
});
